...
try {
    $stmt = $pdo->prepare("
        SELECT a.id, a.title, a.summary, a.status, a.view_count, a.created_at, a.updated_at, a.published_at,
               a.tags, c.name AS category_name
        FROM articles a
        LEFT JOIN categories c ON a.category_id = c.id
        WHERE a.author_id = ?
        ORDER BY FIELD(a.status, 'request', 'published', 'disapproved', 'draft'), a.updated_at DESC
    ");
    $stmt->execute([$author_id]);
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $article) {
        $key = $article['status'] === 'published' ? 'published' : ($article['status'] === 'disapproved' ? 'disapproved' : 'request');
...
<div class="page-body">
  <div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
      <div>
        <h3 class="mb-1" style="font-family: 'Playfair Display', serif; font-weight: 700; color: var(--navy);">Quản lý bài viết tác giả</h3>
        <div class="text-muted" style="font-size:13px;">Theo dõi trạng thái duyệt, lượt xem và chỉnh sửa bài của bạn.</div>
      </div>
    </div>
    ...
    <ul class="nav nav-tabs mb-3" id="writerArticleTabs" role="tablist">
      <li class="nav-item" role="presentation">
        <button class="nav-link active" id="pending-tab" data-bs-toggle="tab" data-bs-target="#pending-pane" type="button" role="tab">
          Chờ duyệt <span class="badge text-bg-warning ms-1"><?= count($articles_by_status['request']) ?></span>
        </button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="nav-link" id="published-tab" data-bs-toggle="tab" data-bs-target="#published-pane" type="button" role="tab">
          Đã xuất bản <span class="badge text-bg-success ms-1"><?= count($articles_by_status['published']) ?></span>
        </button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="nav-link" id="rejected-tab" data-bs-toggle="tab" data-bs-target="#rejected-pane" type="button" role="tab">
          Bị từ chối <span class="badge text-bg-secondary ms-1"><?= count($articles_by_status['disapproved']) ?></span>
        </button>
      </li>
    </ul>
    ...
    <div class="tab-content">
      <?php
      $panes = [
          'pending' => ['status' => 'request', 'label' => 'Danh sách bài viết chờ duyệt', 'active' => true],
          'published' => ['status' => 'published', 'label' => 'Danh sách bài viết đã xuất bản', 'active' => false],
          'rejected' => ['status' => 'disapproved', 'label' => 'Danh sách bài viết bị từ chối', 'active' => false],
      ];
      ...
      <div class="tab-pane fade <?= $pane['active'] ? 'show active' : '' ?>" id="<?= $pane_id ?>-pane" role="tabpanel" tabindex="0">
        <div class="card shadow-sm border-0 p-4 bg-white">
          <span class="section-label"><?= htmlspecialchars($pane['label']) ?></span>
          <?php if (empty($items)): ?>
                ...
                <?php foreach ($items as $article): ?>
                  <tr>
                    ...
                    <td class="text-center">
                      <span class="badge bg-light text-dark border"><i class="bi bi-eye me-1"></i><?= number_format((int)$article['view_count']) ?></span>
                    </td>
                    <td style="font-size:12px;color:var(--muted);min-width:150px;">
