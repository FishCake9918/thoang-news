...
$cache_ttl = 120; // Cache ngắn để vị trí hiện tại cập nhật nhanh hơn

// Tạo key cache duy nhất dựa trên vị trí
$cache_key = $lat && $lon
    ? 'weather_openmeteo_' . round((float)$lat, 3) . '_' . round((float)$lon, 3)
    : 'weather_openmeteo_' . str_replace([' ', ','], '_', strtolower($city));
